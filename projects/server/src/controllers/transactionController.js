...
module.exports = {
  ...
  addTransaction: async (req, res) => {
    ...
    try {
      ...
      const branchStoreLocation = branchStores.findAll({
        attributes: ["id", "latitude", "longitude"],
    })

    const distances = []

    for(let i = 0; i < branchStoreLocation.length; i++){
        const branchLocation = {
            lat: branchStoreLocation[i].latitude,
            lng: branchStoreLocation[i].longitude
        }
